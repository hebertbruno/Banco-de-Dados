...
class Model {
    ...
    Model( char horizontal = '-', char vertical = '|', char corner = '+' ) :
...
std::ostream & operator<<( std::ostream & stream, Model const & table )
{
    table.setup();
    stream << table.ruler() << "\n";
    for ( auto rowIterator = table.rows().begin(); rowIterator != table.rows().end(); ++ rowIterator ) {
        Model::Row const & row = * rowIterator;
        stream << table.vertical();
        for ( unsigned i = 0; i < row.size(); ++i ) {
            auto alignment = table.alignment( i ) == Model::Alignment::LEFT ? std::left : std::right;
